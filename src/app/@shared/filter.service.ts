...
export class FilterService {
  ...
  public filter() {
    ...
    if (filters.rarities!.length > 0) {
      filteredCards = this.filterByRarity(filteredCards, filters.rarities);
    }
  ...
  private setRarityList(cards: DbsCard[]): void {
    const rarities: string[] = _.orderBy(_.uniq(_.flattenDeep(cards.map((card) => card.rarity))));
    this.filterOptions$.next({
      ...this.filterOptions$.value,
      rarityList: _.orderBy(rarities),
    });
  }
  ...
  private filterByRarity(cards: DbsCard[], rarities: string[] | null | undefined): DbsCard[] {
    return cards.filter((card) => {
      if (card.rarity !== undefined && rarities) {
        return rarities.some((rarity) => rarity === card.rarity);
      } else {
        return false;
      }
    });
